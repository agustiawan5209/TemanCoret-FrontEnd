<template>
    <div class="w-full">
        ...
        <div class="w-full relative flex">
            ...
            <input type="search" ref="search" id="search-value" name="search-value" v-model="search"
        ...
        <teleport to="body">
            ...
            <transition name="custom-classes" enter-active-class="animate__animated animate__bounceInLeft"
                ...
                <div class="max-w-full h-screen w-full flex inset-0 justify-center items-start py-10 fixed z-50"
                    @focus="modalSearch = false" @focusout="modalSearch = false" v-show="modalSearch">
                    <div :class="maxWidthClass"
                        class="bg-secondary w-full mx-auto rounded-lg shadow transform transition-all relative ">
                        <div class="w-full max-w-full relative flex">
                            <span class="absolute left-4 top-2 md:top-2 text-lg text-gray-400">
                                <i class="fa-solid fa-magnifying-glass"></i>
                            </span>
                            <input type="search" ref="input" id="input-datasearch" name="input-datasearch" v-model="dataSearch"
                        ...
                        <div class="px-2 pt-3 pb-20 overflow-y-auto h-full max-h-[600px]">
                            <h4 class="text-sm text-gray-600 capitalize">Hasil Pencarian</h4>
                            <ul class="list-item pt-2 space-y-3" v-if="product.length > 0">
                                <li class="w-full grid grid-cols-12" v-for="(item, idx) in product" :key="item.id"
                                    :index="idx">
                                    <a href="#" @click="pushToDetail(item)"
                                        ...
                                        <table
                                            class="table-auto border-collapse w-full max-w-sm text-left text-gray-600 text-sm">
                                            <tr v-for="(item, key) in parseJ(item.productdetails)" :key="item.id"
                                                :index="key">
                                                <th class="py-2 px-4 text-xs sm:text-sm border border-gray-300 w-40 font-medium">{{ key }}</th>
                                                <th class="py-2 px-4 text-xs sm:text-sm border border-gray-300 ">
...
<style>@import "https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css";</style>